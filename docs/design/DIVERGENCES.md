...
|---|---|---|---|---|
| Sélection de vue par rôle (une seule section visible à la fois) | Sélecteur segmenté, une vue exclusive par défaut | `SectionKpi` « Mes dossiers initiés » + « Mes dossiers à traiter » **toujours empilées**, jamais de choix exclusif (aucune des deux gated sur rôle) | **Défaut d'implémentation trouvé, non corrigé dans cette passe** (portée : inventaire, pas reconstruction d'écran) | Non corrigé — le commentaire du code (« aucune tuile masquée par rôle… règle non négociable 2 ») justifie l'absence de masquage à des fins de **sécurité** (R2 : le client ne doit jamais être la seule garantie), mais R2 ne dit rien sur la réduction du bruit visuel pour un rôle donné — les deux questions ont été conflées. Rester empilé n'est ni une catégorie 1-5 ni une invention de structure (contrairement à la Sidebar ci-dessus) : `profil=initiateur`/`profil=valideur` existent réellement et scoperaient une vue unique sans rien inventer. À décider séparément (reconstruction d'écran, hors périmètre de cet inventaire) |
...
**Ligne à retenir** : sur quatre écarts trouvés, trois sont déjà couverts par des décisions existantes (KPI scope, vues matérialisées, `engine.jsx` jamais porté) — un seul (l'empilement systématique au lieu d'un choix de vue par rôle) est un vrai défaut non corrigé, laissé pour une décision de reconstruction distincte, pas actée par défaut comme « la bonne décision » sans preuve.
