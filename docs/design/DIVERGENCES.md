...
**Chantier fonctionnel, pas un défaut de portage visuel** : construire ces champs correctement exige d'abord de compléter le backend (les quatre endpoints de liste référentielle ci-dessus, désormais faits), avant tout ajout côté écran — de la taille d'une petite phase API, pas d'un ajustement de mise en page. **Décomposé en phases** (Phase A backend référentiels, close ; B/C/D champs par circuit, à venir ; E « mémo » DF sans source réelle, question ouverte ; G `numeroCase`/JADE, écart D8 CLAUDE.md) : l'audit visuel de cet écran continue à périmètre inchangé en attendant (les 4 champs déjà construits, comparés à la maquette pour la mise en page, les couleurs et la structure — pas le nombre de champs).
