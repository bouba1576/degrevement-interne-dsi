...
- **Aucune route ne sert le fichier réel d'une pièce jointe (trouvé en construisant `PiecesTab`, `DossierDetailScreen`, Phase 9.2).** `POST /api/demandes/{id}/pieces` et `DELETE .../pieces/{pieceId}` existent ; `GedStubAdapter.stocker` écrit un vrai fichier sur disque (`GED_STORAGE_PATH`) — mais aucune route ne le lit. `PiecesTab` affiche nom/type/taille et permet ajout/suppression, sans lien de téléchargement : absence constatée, pas un lien mort ajouté en devinant une route. À traiter comme sa propre étape (probablement `GET /api/demandes/{id}/pieces/{pieceId}` en streaming, avec la même ouverture que le reste du dossier — docs/06 §4 — puisque la pièce appartient à un dossier déjà lisible).
- **Déléguer (`POST /api/taches/{id}/deleguer`) non câblé dans `TacheActionBanner` (Phase 9.2) — route réelle, UI manquante faute d'un mécanisme de recherche d'agent.** La route exige `delegataireId` (uuid) ; aucune route n'existe pour rechercher un agent par nom ou identifiant AD et résoudre son id. Un champ texte demandant un UUID brut aurait été une UX qui ne fonctionne pas, pas un raccourci acceptable — omis plutôt que construit sur une supposition. À traiter comme sa propre étape (recherche d'agent, probablement bornée aux membres du rôle concerné plutôt qu'un annuaire ouvert — même question de portée que la résolution de nom pour `agentClaimId`, cf. mécanisme de claim ci-dessous).
