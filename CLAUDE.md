...
**Test de concurrence abandonné, pas masqué** : une tentative de couvrir le catch `P2002` de `assignerFormuleCourante` par deux appels concurrents réels (même schéma que T1) s'est révélée flaky (3 échecs sur 5 relances) — la contrainte partielle `uq_formule_courante` ne produit pas de conflit de façon fiable sous l'isolation READ COMMITTED par défaut de Postgres à cette vitesse. Retiré plutôt que gardé « la plupart du temps vert » (même discipline que l'incident `calendrier_sla`, Phase 5 : un test qu'on ne peut pas faire confiance à chaque run n'a pas sa place dans la suite). Les lignes 96-102 (le catch lui-même) restent non couvertes par ce chemin précis — le comportement R19 reste vérifié par ailleurs, au niveau de la contrainte base directement.

**`demandes.controller.ts`** : 57.74 % → **100 %** statements/lignes/fonctions (73.46 % branches). Complété en deux temps : les trois e2e par circuit (10.3) couvraient déjà `creer`/`definirLignes`/`apercuRoutage`/`soumettre`/`obtenirDetail`/`listerTaches` en HTTP réel, faisant mécaniquement remonter la couverture de 57.74 % à 74.64 % avant tout travail dédié. `demandes-controller.e2e-spec.ts` (nouveau) ferme le reste : `lister` (scopé `profil=initiateur`), `DELETE` (brouillon), `PATCH` (modification simple), `POST /calcul` (recalcul), `POST /abandonner`+`/rappeler` (avec la découverte, en écrivant le test, que ces deux routes **rejettent un brouillon**, 422 `DEMANDE_NON_ELIGIBLE` — `terminerSiAucuneDecision` exclut explicitement `BROUILLON`/`ABANDONNE`/`VALIDE`/`REJETE`, la suppression d'un brouillon est `DELETE`, une opération distincte déjà actée en Phase 9.2 ; l'hypothèse initiale du test, un brouillon abandonnable, était fausse, corrigée en soumettant réellement le dossier avant d'appeler ces routes), pièces jointes (upload/suppression multipart), état SI (lecture + rejeu manuel `ADMIN_PGD`-only avec la découverte que `siEtat` par défaut est `EN_ATTENTE`, pas une valeur "non applicable" supposée), et le 404 de `apercu-routage` sur un id inconnu.
