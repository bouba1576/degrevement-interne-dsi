...
- **Suppression d'un brouillon par son initiateur — n'existe pas, trouvé en vérification live de `NouvelleDemandeScreen` (Phase 9.2). Priorité élevée : confirmé que ça pollue les KPI, pas une question théorique.** `POST /api/demandes/{id}/abandonner` **exclut explicitement** `BROUILLON` de son périmètre (`DemandeWorkflowService.terminerSiAucuneDecision` : `BROUILLON`/`ABANDONNE`/`VALIDE`/`REJETE` → 422 `DEMANDE_NON_ELIGIBLE`) — vérifié en direct contre un vrai brouillon créé via l'écran. C'est un choix juste, pas un bug : « abandon » a un sens métier précis (renoncer à un dossier déjà engagé dans une chaîne de validation, transition tracée), qu'un brouillon jamais soumis n'a pas encore. Confondre les deux ferait compter des brouillons jamais soumis dans un taux d'abandon — un indicateur faux, pas un raccourci acceptable. Il manque donc une opération **distincte** : « suppression d'un brouillon », pas un abandon déguisé. **Confirmé également, ce qui referme la question posée plus haut sur la pollution** : un `BROUILLON` orphelin est inerte côté corbeilles (`GET /api/taches` — aucune `Tache` n'existe avant l'instanciation à la soumission, `RuleEngineService` n'est appelé que dans `soumettre()`), mais **pollue réellement** la famille KPI « reçus » (`RECUS_VOLUME`/`RECUS_MONTANT_HT`/`RECUS_MONTANT_TTC` et leurs variantes `_PAR_UNIVERS`/`_EVOLUTION`) — `KpiEngineService.construireWhere` ne filtre par aucun statut, comportement **testé et volontaire** (`kpi-engine.integration.spec.ts` exige qu'une demande fraîchement créée compte immédiatement), vérifié aussi en direct (créer un brouillon a fait passer `RECUS_VOLUME` de 0 à 1 sans autre action). La famille « traités » reste inerte (`surDossiersTraites` force `siEtat=CONFIRME`, jamais atteint par un brouillon). Palliatif appliqué côté écran en attendant : `NouvelleDemandeScreen` ne crée plus la demande au formulaire minimal, seulement au premier `PUT .../lignes` réussi — réduit le nombre de brouillons orphelins sans rien changer côté serveur, ne résout pas le cas où l'utilisateur quitte APRÈS avoir enregistré au moins une ligne. **Points à trancher pour l'étape API dédiée** (même lot que `GET /api/notifications` et le filtre « mes demandes » ci-dessus) : suppression physique de `Demande`/`DemandeLigne`/pièces déjà attachées, ou statut dédié (`SUPPRIME` ?) conservé pour trace ; trace en `JournalAudit` ou non (un brouillon jamais soumis n'a peut-être pas vocation à laisser une trace d'audit) ; garde de portée (l'initiateur uniquement, comme `InitiateurDemandeGuard` déjà posé sur les autres routes de ce contrôleur) — et sa propre entrée dans `apps/api/test/guard-coverage.spec.ts`.
