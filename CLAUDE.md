...
**Dette de couverture réelle sur une phase considérée close — trouvée en vérifiant le nombre de paliers DF (Phase 10.6, décomposition NouvelleDemandeScreen), pas en auditant 10.3 pour elle-même.** Le parcours manuel de clôture de Phase 9 avait exercé le palier intermédiaire (dossier `DF-2026-AF5715`, 6,5M HT), et `df-circuit.e2e-spec.ts` a été écrit contre ce même cas — **un seul des trois paliers réels de DF a donc jamais été exercé de bout en bout, ni manuellement ni automatiquement.** Les paliers 1 (0–5M, 3 étapes, aucune validation `DF` ni contrôle FRA) et 3 (>50M, 6 étapes avec `DGA_DG`) n'ont aucune couverture e2e — pas un défaut de code (le test reflète fidèlement le seul cas déjà vérifié à la main), mais une dette de couverture non comblée à ce jour. Le socle d'identités persistantes a le même trou, comblé après coup (cf. section dédiée, `dga.dg@orange.ci`) ; l'e2e, lui, reste à écrire.
