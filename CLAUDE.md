...
### R3 (SoD) s'applique aussi au contrôle a posteriori FRA, pas seulement à la chaîne de validation bloquante — trouvé lors de l'essai de bout en bout (Phase 9, clôture)
...
**Conséquence pratique pour la suite des vérifications live** : réutiliser une seule identité de test à travers toute une chaîne DF (comme fait par commodité dans des vérifications précédentes plus ciblées) fonctionne pour les étapes de vérification classiques mais échoue silencieusement — enfin, pas silencieusement, avec un vrai 403 — dès que cette identité touche à la fois la dernière étape bloquante et le contrôle FRA qui la suit. Une identité distincte est nécessaire pour le contrôle dès que l'acteur de l'étape précédente est connu.

**Effet de bord découvert en construisant cette identité distincte** : `Role.requiertMfa` détermine si une session complète nécessite un second facteur, mais `Utilisateur.mfaMethode` a pour défaut `DUO` (`schema.prisma`) — DUO indisponible en environnement dev (`503 MFA_INDISPONIBLE`), déjà documenté ailleurs. Un utilisateur fraîchement provisionné (aucun groupe AD, donc aucun rôle `requiertMfa`) peut obtenir une session et s'auto-enrôler en TOTP (flux déjà documenté, cf. Questions ouvertes), mais **rien ne fait ensuite passer `mfaMethode` de `DUO` à `TOTP`** — aucune route, admin ou self-service, ne modifie ce champ. Contourné ici par une écriture DB directe (légitime pour un besoin de vérification ponctuel, pas un correctif produit). À rapprocher de l'entrée « Questions ouvertes » sur l'enrôlement TOTP : la lacune est en réalité double — pas seulement « comment enrôler le secret », mais aussi « comment signaler que ce compte utilise désormais TOTP plutôt que DUO ».

**Ne pas** : conclure que R3 est mal implémenté — le comportement observé est cohérent avec la lettre de la règle (« un agent ne valide pas l'étape N s'il a agi à l'étape N-1 du même dossier », sans exception pour les étapes de contrôle) et a empêché exactement le scénario qu'il est censé empêcher. C'est une confirmation, pas un bug.
