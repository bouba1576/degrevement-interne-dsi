...
// « Restauration transitoire — AdApiProvider »). Aucun accès réseau réel
// possible depuis cet environnement de développement vers l'API AD réelle
// (192.168.31.78, cf. CLAUDE.md « API AD réelle ») — serveur HTTP local qui
// reproduit exactement la forme documentée du contrat, plus les cas non
// documentés que la règle d'échec fermé doit couvrir explicitement. Un seul
// serveur pour tout le fichier : AD_API_URL est fixé une seule fois avant le
// premier appel à loadEnv() (mémoïsé, cf. packages/config/src/env.ts) — la
// réponse varie par test via `prochaine`, jamais par variable d'environnement.
describe("AdApiProvider (serveur HTTP simulé, contrat API AD réelle)", () => {
  ...
  // cf. keycloak.port.ts), les deux vérifiés explicitement.
  ...
  it("department capturé et journalisé, jamais retourné ni utilisé pour dériver une direction", async () => {
    ...
    const resultat = await provider.authentifier("x", "x");
    // UtilisateurAd (apps/api/ports/keycloak.port.ts) n'a que
